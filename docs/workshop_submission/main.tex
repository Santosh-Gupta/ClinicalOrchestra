...
\title{Ranked Differential Diagnosis Across Frontier LLMs:\\ A Contamination-Controlled Neurology and Psychiatry Benchmark with a Retrieval Harness}
...
it lower in a broader list, so the model ordering re-arranges as the rank cutoff grows. Because these
models' training data and post-training are undisclosed, we treat mechanistic explanations as conjecture.
We then present ClinicalHarness, a retrieval-guided pipeline that pairs an LLM with PubMed/PMC retrieval and
a cheap per-paper reader, grounding each diagnosis in citable literature, and show that it improves top-5
ranked accuracy for all three answerers we could run at pipeline scale, most where the base model is weakest.
\end{abstract}
...
These systems are meant to \emph{support} a clinician rather than to diagnose autonomously, and in that
setting a single best guess is often less useful than a short ranked list of plausible diagnoses the
clinician can weigh. We therefore score the whole ranked differential (top-1 through top-5) and ask two
...
Flash)---consistent with retrieval helping most where the base model has a genuine knowledge gap. Each
differential is grounded in the papers the reader retained as relevant---a mean of roughly 8--12 per case
(standard deviation 7--10), which a clinician can look up---screened from a much larger retrieved pool: the
reader rejects a mean of 26--30 papers per case (standard deviation $\sim$12) as irrelevant, out of
$\sim$34--41 retrieved. (Substituting the
harness's reordered differential wholesale instead of fusing it \emph{lowers} ranked accuracy, by crowding
the model's own correct diagnoses down the list; \S\ref{sec:disc}.)
