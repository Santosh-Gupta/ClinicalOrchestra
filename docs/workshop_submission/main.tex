...
\maketitle
\lhead{}% suppress the "Preprint. Under review." running header on all pages
...
published case reports into diagnostic challenges, then filter out challenges with answer leakage and those
whose diagnosis is not derivable from the prompt when checked against the source case report it was created
from; every retained case is a report published after every evaluated model's training cutoff, so it cannot
have been seen in pretraining. On a 68-case hard set
(cases DeepSeek V4 Flash fails closed-book), we evaluate seven frontier models---DeepSeek V4 Flash, DeepSeek
V4 Pro, Gemini 3.5 Flash, Gemini 3.1 Pro, Claude Opus 4.8, GPT-5.4, and GPT-5.5---and report how they
distribute correct answers across ranks. The central finding is that models differ not only in accuracy but in \emph{where they
place the correct diagnosis}: some surface it as their most confident (top-1) prediction, while others uncover
it lower in a broader list, so the model ordering changes as the rank cutoff grows.
...
can verify: a cheap reader screens the papers retrieved for each case and keeps only the relevant ones,
rejecting most. It improves top-5 ranked accuracy for all three answerers we could run at pipeline scale.
\end{abstract}
...
assertion. Most evaluations reward neither: they score a single label with no provenance. We therefore score
the whole ranked differential (top-1 through top-5) and ask two
...
grounds each diagnosis in citable PubMed/PMC sources, and the finding that naive retrieval integration can
\emph{degrade} a frontier model's ranked accuracy while a do-no-harm fusion improves it for every answerer we
...
\begin{figure}[t]
  ...
  before a hand-checked drop/mend pass. Each stage is described in \ref{app:bench-components}.}
  \label{fig:benchmark}
\end{figure}
...
sequencing result; in such cases the gene is not inferable from a phenotype shared with near neighbors (such
as DJ-1/PARK7 versus PRKN versus PINK1 in autosomal-recessive early-onset Parkinson's). A genuine reasoning
...
\paragraph{Repair or drop.} DeepSeek V4 Pro was used to validate determinacy: it reads the challenge, the
gold, and the source full text and flags non-determinate cases. Each flagged case is then either repaired
from the source---adding the missing deciding result (the sequencing result, antibody titer, or biopsy
finding) so the gold becomes
...
\paragraph{Why a ranked differential.} For the near-neighbor cases above, the correct entity is often
...
penalizing it for the granularity of the single \#1 choice. We score top-1 through top-5 throughout.

\paragraph{Composition.} Table~\ref{tab:compose} characterizes the set, and Figure~\ref{fig:topics} shows
its etiologic-class frequencies. The set is intentionally broad: cases span a wide range of conditions, and
across the 64 source papers carrying author keywords there are 338 distinct keywords with almost no
repetition. Patients
...
judge. The set is failure-selected by an earlier closed-book DeepSeek V4 Flash filter (distinct from this
ranked-differential run), so absolute numbers index difficulty, not general quality. Bold marks the two best
top-5 scores.}
\label{tab:bare}
\end{table}
...
\paragraph{The top-1 to top-2 jump separates the spread models.} The per-rank gains expose the source of the
crossover.
...
with ranks: DeepSeek V4 Flash defines the set under that earlier closed-book filter, yet under the
ranked-differential prompt it recovers 40/68 at top-5, so for much of its closed-book failures the knowledge
was in the differential and the failure was one of selection. Notably, and to our surprise, the smallest and
cheapest model (Gemini Flash) climbed from sixth at top-1 to second at top-5, and the earlier GPT-5.4 edged
the later GPT-5.5; but the training data and
...
text and can effectively read many papers per case through a smaller context.
...
neurological emergency, infection/inflammation, or rare tumor), which conditions the prompt toward the
relevant discriminators.

\paragraph{2. Query generation (primary LLM).} From the problem representation the primary LLM proposes
targeted PubMed/PMC queries. These favor \emph{discriminator} queries that separate the leading hypothesis
...
\paragraph{4. Per-paper screening (reader LLM, DeepSeek V4 Flash).} Each retrieved paper is screened in its
own isolated DeepSeek V4 Flash reader call. The reader returns a compact structured note only
when the paper is judged relevant to \emph{this} case---a short relevant excerpt, the discriminators it supplies, which hypotheses it supports or
...
single evidence pack that summarizes the decisive discriminators and the still-unresolved mimic pairs, and
decides whether another retrieval round is warranted. When a discriminator remains unresolved, the system
issues a follow-up query plan and runs another round (up to a bounded maximum); otherwise it finalizes.
...
no component was optimized on the evaluation cases.

\section{Harness Results: Bare versus Harness (top-5)}\label{sec:harnessresults}

We compare each model's bare ranked differential (Table~\ref{tab:bare}) against its do-no-harm \emph{fused}
ranked differential, with the model as the primary LLM paired with the cheap DeepSeek reader and the same
majority-of-3 judge, on the same 68 cases. Both sides are temperature 0.
...
contributes only at the margin, recovering a missed diagnosis into the last slot. This improves top-5
for all three answerers (Table~\ref{tab:harness}); across these three, the top-5 gain is largest for the
weakest base model---$+1$ for the strongest (Gemini Flash), $+2$ for DeepSeek V4 Pro, and $+3$ for the
weakest (DeepSeek V4 Flash)---consistent with retrieval helping most where the base model has a genuine
knowledge gap.
...
ones with the specific discriminators they supply. In these runs this yields a mean of about 10 grounding
sources per case (standard deviation $\sim$9.5, range 0--39), selected from a mean of $\sim$39 retrieved
papers of which the reader rejects roughly 73\% as irrelevant. These retained papers, attached to the final differential by PMCID/DOI,
...
Second---and the result that most surprised us---augmentation can \emph{hurt}. We did not set out to build a
do-no-harm fusion; we arrived at it because more direct designs, in which the harness re-ranks or replaces
the model's own differential, repeatedly \emph{lowered} ranked accuracy relative to the bare model. The
implication is broader than our pipeline: a frontier LLM already carries rich internal medical knowledge and
a wide implicit differential, and grounding its answer in a handful of retrieved passages can crowd that
knowledge out---so that adding correct information makes the system worse. Retrieval helped here only once we
constrained it never to override the model's confident candidates and to contribute precisely where the model
had a gap. We do not claim to have solved this. The harness is still in development, and learning to use
retrieval to \emph{support} rather than displace a strong model's reasoning---especially near the top of the
ranking, where the model's own judgement is usually right---is the open problem. The gains in
Table~\ref{tab:harness} are the conservative floor of careful integration, not its ceiling.
...
trial. The harness fusion is deliberately conservative---it touches only the fifth slot---and is a work in progress:
it is the safe floor we could establish, not a finished method for integrating retrieval with a frontier
model's reasoning, and a less restrictive integration risks reintroducing the degradation that more direct
designs showed.
...
the missing deciding result (a sequencing result, antibody titer, or biopsy finding) or reframing to the
